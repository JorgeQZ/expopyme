<?php
    ...
    $args = array(
        ...
        'menu_title'           => __( 'ExpoPyme Options', 'redux-framework-demo' ),
        'page_title'           => __( 'ExpoPyme Options', 'redux-framework-demo' ),
    ...
    foreach ($_SESSION['my_posts'] as $key => $value) {
        $all_posts[] = $value;
    }
    ...
    for ($i=0; $i < sizeof( $all_posts ); $i++) { 
        ...
        $args = array('posts_per_page'=> -1,'post_type' => $all_posts[$i]);
        $page_query = new  WP_Query( $args );
        if( $page_query->have_posts() )
        while ( $page_query->have_posts() ) : $page_query->the_post();
            $past_ids[$i][get_the_id()] = get_the_title();
        endwhile;
        wp_reset_query();
    ...
    $f_array[] =  array('id' => 'opt-multi-select-pages-n',
        'type'     => 'select',
        'data'     => 'pages',
        'multi'    => true,
    ...
    for ($i=0; $i < sizeof( $all_posts ); $i++) { 
        ...
        $f_array[] = array(
                'id'       => 'opt-multi-select-posts-n'.$i,
                'type'     => 'select',
                'options'  => $past_ids[$i],
                'multi'    => true,
                'title'    => __( $all_posts[$i].' Multi Select Option', 'redux-framework-demo' ),
                'subtitle' => __( 'No validation can be done on this field type', 'redux-framework-demo' ),
                'desc'     => __( 'This is the description field, again good for additional info.', 'redux-framework-demo' ),
            );
    ...
    Redux::setSection( $opt_name, array(
        ...
        'id'     => 'typographya',
        'desc'   => __( 'It excludes results page.', 'redux-framework-demo' ) . '',
        'icon'   => 'el el-edit',
        'fields' => $f_array
    ) );
    ...
    Redux::setSection( $opt_name, array(
        'title'      => __( 'WPML ExpoPymetion', 'redux-framework-demo' ),
        'desc'       => __( 'These fields can be fully translated by WPML (WordPress Multi-Language). This serves as an example for you to implement. For extra details look at our <a href="http://docs.reduxframework.com/core/advanced/wpml-expopymetion/" target="_blank">WPML Implementation</a> documentation.', 'redux-framework-demo' ),
        'subsection' => true,
        // 'submenu' => false, // Setting submenu to false on a given section will hide it from the WordPress sidebar menu!
        'fields'     => array(
            array(
    ...
    // Remove the demo link and the notice of expopymeted demo from the redux-framework plugin
